...
def _tg_username_key(tg_id: int) -> str:
    return "tg_%s" % int(tg_id)


def _ensure_tg_user(tg_obj: dict):
    """Create/update SaaS user keyed by Telegram id. Admin if id in ADMIN_TG_IDS."""
    if not tg_obj or tg_obj.get("id") is None:
        return None
    tg_id = int(tg_obj["id"])
    key = _tg_username_key(tg_id)
    users = _load_users()
    u = users.get(key)
    is_admin = tg_id in ADMIN_TG_IDS
    name = " ".join([str(tg_obj.get("first_name") or ""), str(tg_obj.get("last_name") or "")]).strip() or (tg_obj.get("username") or key)
    if not u:
        ...
        u = {
            ...
            "active": True if is_admin else False,  # admin always on; others need grant
            "tg_id": tg_id,
            "tg_username": tg_obj.get("username") or "",
            "display_name": name,
        ...
        u["tg_id"] = tg_id
        u["tg_username"] = tg_obj.get("username") or u.get("tg_username") or ""
        if name:
            u["display_name"] = name
        if is_admin:
            u["role"] = "admin"
            u["active"] = True
            if not u.get("plan") or u.get("plan") == "free":
                u["plan"] = "proplus"
...
ADMIN_TG_IDS = set()
for _part in str(os.getenv("ADMIN_TG_IDS", "5427735251") or "5427735251").split(","):
    _part = _part.strip()
    if _part.isdigit():
        ADMIN_TG_IDS.add(int(_part))
if not ADMIN_TG_IDS:
...
FOD_POOL = [
    ...
    {"id": "flat180", "title": "₹180 OFF", "text": "₹180 OFF", "subtitle": "Flat ₹180 off on your 1st order (new account)", "pct": 0, "flat": 180, "duration": 3, "savings": "₹180"},
...
# ---- anti-block HTTP layer (proxy + pacing + retries) ----
_MEESHO_PROXY = (
    os.getenv("MEESHO_HTTP_PROXY")
    or os.getenv("HTTPS_PROXY")
    or os.getenv("HTTP_PROXY")
    or ""
).strip()
_MEESHO_MIN_GAP = float(os.getenv("MEESHO_MIN_GAP", "0.35") or 0.35)
_last_meesho_call = 0.0


def _proxy_map():
    if not _MEESHO_PROXY:
        return None
    return {"http://": _MEESHO_PROXY, "https://": _MEESHO_PROXY}


async def _pace():
    global _last_meesho_call
    now = time.time()
    gap = _MEESHO_MIN_GAP - (now - _last_meesho_call)
    if gap > 0:
        await asyncio.sleep(gap + random.uniform(0.05, 0.2))
    _last_meesho_call = time.time()


def _is_block_status(code):
    return int(code or 0) in (403, 429, 451, 462, 503)


def _block_message(resp):
    try:
        body = (resp.text or "")[:280]
    except Exception:
        body = ""
    code = getattr(resp, "status_code", "?")
    if code in (403, 451):
        return (
            f"Meesho blocked this server IP (HTTP {code}). "
            "Set env MEESHO_HTTP_PROXY to an India residential proxy and redeploy."
        )
    if code == 429:
        return "Meesho rate-limited (HTTP 429). Wait a minute or use a fresh proxy."
    if code == 462:
        return "Meesho session rejected (HTTP 462). Re-login that Meesho number."
    return f"Meesho error HTTP {code}: {body}"


async def meesho_request(method, url, *, headers=None, json=None, params=None, data=None, timeout=25):
    """Live Meesho call with pacing, retries, optional residential proxy."""
    last = None
    hdrs = headers or meesho_headers()
    proxies = _proxy_map()
        ...
        try:
            ...
            async with httpx.AsyncClient(**kwargs) as client:
                resp = await client.request(
                    method, url, headers=hdrs, json=json, params=params, data=data
                )
            code = resp.status_code
            if code >= 500 or code in (408, 429, 403, 462):
                last = resp
                await asyncio.sleep(min(6.0, (0.7 * (2 ** attempt)) + random.uniform(0.1, 0.5)))
                continue
            return resp
            ...
            for c in catalogs:
                ...
                def _nz(v):
                    try:
                        ...
                        return 0
                price = (
                    _nz(pv.get("prepaid_price"))
                    or _nz(c.get("min_catalog_price"))
                    or _nz(c.get("min_product_price"))
                    or _nz(c.get("price"))
                    or _nz(c.get("hero_product_price"))
                    or _nz((c.get("price_view") or {}).get("price"))
                    or _nz((c.get("price_view") or {}).get("final_price"))
                    or _nz(c.get("original_price"))
                    or 0
                )
                original = (
                    _nz(c.get("original_price"))
                    or _nz(c.get("mrp"))
                    or _nz(c.get("max_product_price"))
                    or price
                )
                ...
                rev = c.get("catalog_reviews_summary") or c.get("product_reviews_summary") or c.get("reviews_summary") or {}
                avg_r = rev.get("average_rating") or rev.get("avg_rating") or c.get("average_rating") or c.get("rating")
                cnt_r = rev.get("rating_count") or rev.get("count") or c.get("rating_count") or 0
                try:
                    avg_r = float(avg_r) if avg_r is not None else None
                except Exception:
                    avg_r = None
                try:
                    cnt_r = int(cnt_r or 0)
                except Exception:
                    cnt_r = 0
                fod = None
                ...
                out.append({
                    ...
                    "rating": {"average": avg_r, "count": cnt_r, "score": avg_r},
                    "rating_count": cnt_r,
...
async def meesho_product(product_id):
    ...
    def _pos(v):
        try:
            ...
            return 0.0
    final = (
        _pos(pv.get("prepaid_price"))
        or _pos(dp.get("min_product_price"))
        or _pos(dp.get("price"))
        or _pos(sup.get("price"))
        or _pos(sup.get("final_price"))
        or _pos(p.get("price"))
        or _pos(p.get("mrp"))
        or _pos((p.get("price_view") or {}).get("final_price"))
        or 0.0
    )
    mrp = (
        _pos(sup.get("original_price"))
        or _pos(p.get("mrp"))
        or _pos(dp.get("mrp"))
        or _pos(dp.get("max_product_price"))
        or final
    )
    ...
    rating = (
        sup.get("average_rating")
        or (sp or {}).get("average_rating")
        or p.get("average_rating")
        or ((p.get("catalog_reviews_summary") or {}).get("average_rating"))
        or ((p.get("product_reviews_summary") or {}).get("average_rating"))
    )
    rating_count = (
        sup.get("rating_count")
        or (sp or {}).get("rating_count")
        or p.get("rating_count")
        or ((p.get("catalog_reviews_summary") or {}).get("rating_count"))
        or ((p.get("product_reviews_summary") or {}).get("rating_count"))
        or 0
    )
    try:
...
def _default_settings():
    return {
        "per_order_price": float(os.getenv("PER_ORDER_PRICE", "0") or 0),
        "admin_upi_id": os.getenv("ADMIN_UPI_ID", "").strip(),
        "admin_upi_name": os.getenv("ADMIN_UPI_NAME", "SJ Shop").strip() or "SJ Shop",
        "shop_name": os.getenv("SHOP_NAME", "SJ Shop").strip() or "SJ Shop",
    }
    ...
    try:
        if os.path.exists(SETTINGS_FILE):
            ...
            if isinstance(data, dict):
                s.update({k: data[k] for k in data if k in s or k in ("per_order_price", "admin_upi_id", "admin_upi_name", "shop_name")})
    except Exception:
        pass
    try:
...
def _ensure_admin():
    ...
    if user and user not in users:
        users[user] = {"id": 1, "username": user, "password": pwd, "role": "admin",
                       ...
                       "last_seen": int(time.time()), "used": {"date": "", "count": 0}, "trial_used": 0, "balance": 0}
        _save_users(users)
    return users
...
_PUBLIC_PATHS = ("/", "/api", "/api/bootstrap", "/api/auth/login", "/api/auth/me",
                 ...
                 "/api/auth/json_login", "/api/auth/telegram", "/api/saas/plans")
...
def _debit_wallet(user, amount):
    """Deduct per-order shop fee from user wallet. Returns (ok, new_balance, message)."""
    try:
        amount = float(amount or 0)
    except Exception:
        amount = 0.0
    if amount <= 0:
        return True, float((user or {}).get("balance") or 0), "no_fee"
    users = _load_users()
    u = users.get((user or {}).get("username")) if user else None
    if not u:
        return False, 0.0, "no_user"
...
async def device_isolation_middleware(request: Request, call_next):
    ...
    try:
        ...
        did = "default"
    sess = (request.headers.get("X-Session") or "").strip()
    init_data = (request.headers.get("X-Tg-Init-Data") or "").strip()
    username = _verify_token(sess) if sess else None
    user = None
    users = _ensure_admin()
    if username:
        user = users.get(username)
    # Telegram WebApp: auto-resolve user from initData when no session cookie/token
    if not user and init_data:
        tg_obj = _tg_user_obj(init_data)
        if tg_obj:
            user = _ensure_tg_user(tg_obj)
            users = _load_users()
            user = users.get(_tg_username_key(int(tg_obj["id"])))
    if user and not user.get("active") and user.get("role") != "admin":
        # inactive non-admin: treat as logged-in but blocked for gated APIs
        pass
    if user:
        ...
        try:
            ...
            if user.get("username") in users:
                users[user["username"]]["last_seen"] = user["last_seen"]
                _save_users(users)
        except Exception:
            pass
    CURRENT_USER = user
    ...
    if admin:
        ...
        if not user:
            ...
            db = DEVICES.setdefault(ns, _default_state())
            return JSONResponse(_auth_err(), status_code=401)
        if user.get("role") != "admin" and not user.get("active"):
            ns = "u%s:%s" % (int(user.get("id") or 0), did)
            CURRENT_NS = ns
            db = DEVICES.setdefault(ns, _default_state())
            return JSONResponse({
                "error": "access_denied",
                "message": "Access not granted. Ask admin to allow your Telegram ID.",
...
# Absolute paths so Render/cwd changes never break static mounts
_BASE_DIR = os.path.dirname(os.path.abspath(__file__))
_PUBLIC_DIR = os.path.join(_BASE_DIR, "public")
_JS_DIR = os.path.join(_PUBLIC_DIR, "UnknownGuy_js")
_CSS_DIR = os.path.join(_PUBLIC_DIR, "UnknownGuy_css")
_INDEX_HTML = os.path.join(_PUBLIC_DIR, "index.html")

for _d in (_PUBLIC_DIR, _JS_DIR, _CSS_DIR, os.path.join(_BASE_DIR, "state")):
    try:
...
async def serve_index():
    if not os.path.isfile(_INDEX_HTML):
        return JSONResponse({"error": "index_missing", "message": "public/index.html not found on server"}, status_code=500)
    return FileResponse(_INDEX_HTML, headers={"Cache-Control": "no-store, no-cache, must-revalidate"})
...
async def api_index():
    ...
    return {
        ...
        "version": "1.1",
        "live": True,
        "proxy_configured": bool(_MEESHO_PROXY),
        "admin_tg_ids": sorted(ADMIN_TG_IDS),
        "endpoints": routes,
        "usage": "Telegram WebApp at GET /. Bot: set BOT_TOKEN + SHOP_URL and run python bot.py",
    }
...
async def api_auth_telegram(request: Request):
    """Auto-login from Telegram WebApp initData. Admin = ADMIN_TG_IDS (default 5427735251)."""
    init_data = (request.headers.get("X-Tg-Init-Data") or "").strip()
    body = {}
    try:
        ...
        body = {}
    if not init_data:
        init_data = str((body or {}).get("init_data") or "").strip()
    tg_obj = _tg_user_obj(init_data)
    if not tg_obj:
        return {"ok": False, "error": "no_telegram", "message": "Open this shop from the Telegram bot."}
    u = _ensure_tg_user(tg_obj)
    if not u:
        return {"ok": False, "error": "auth_failed", "message": "Could not create Telegram session."}
    token = _issue_token(u["username"])
    plan = _plan(u)
    return {
        "ok": True,
        "token": token,
        ...
        "plan": plan,
        "access": bool(u.get("active") or u.get("role") == "admin"),
        "message": None if (u.get("active") or u.get("role") == "admin") else "Waiting for admin access. Share your Telegram ID with admin.",
    }


@app.post("/api/admin/grant")
async def api_admin_grant(data: dict = None):
    """Admin grants/revokes order access by Telegram numeric id or tg_ username."""
    u = _current_user()
    if not (u and u.get("role") == "admin"):
        return {"error": "admin_only", "message": "Admins only."}
    data = data or {}
    tg_raw = data.get("tg_id") or data.get("username") or data.get("user")
    if tg_raw is None:
        return {"error": "missing", "message": "Provide tg_id"}
    s = str(tg_raw).strip()
    if s.isdigit():
        key = _tg_username_key(int(s))
        tg_id = int(s)
    elif s.startswith("tg_"):
        key = s
        tg_id = int(s[3:]) if s[3:].isdigit() else None
    else:
        key = s
        tg_id = None
    users = _load_users()
    ...
    if not target:
        if tg_id is None:
            return {"error": "not_found", "message": "User not found. They must open the bot once first."}
        target = {
    ...
    if grant is None:
        grant = data.get("active", True)
    target["active"] = bool(grant)
    if data.get("plan") in PLANS:
        target["plan"] = data["plan"]
    if data.get("add_balance") is not None:
        try:
            target["balance"] = max(0.0, float(target.get("balance") or 0) + float(data.get("add_balance") or 0))
        except Exception:
            pass
    users[key] = target
    _save_users(users)
    return {"ok": True, "username": key, "tg_id": target.get("tg_id"), "active": target["active"],
            "plan": target.get("plan"), "balance": float(target.get("balance") or 0)}
...
def _auth_me(u):
    ...
    user_blob = {
        ...
        "role": cu.get("role"),
        "plan": cu.get("plan"),
        "devices": len(_user_devices(cu)),
        "accounts": _user_account_count(cu),
        "id": cu.get("id"),
        "active": bool(cu.get("active") or cu.get("role") == "admin"),
        "tg_id": cu.get("tg_id"),
        "display_name": cu.get("display_name"),
        "balance": float(cu.get("balance") or 0),
    }
    if plan.get("trial"):
        used = _trial_used(cu)
        cap = TRIAL_FREE_ORDERS
        left = max(0, cap - used)
        return {"authenticated": True, "user": user_blob,
...
async def api_auth_otp_verify(data: dict = None, request: Request = None):
    """Verify OTP and attach Meesho account to the CURRENT Telegram/SaaS user.
    Does not force a second login when already authenticated via Telegram."""
    data = data or {}
    phone = str(data.get("phone_number", "")).strip()[-10:]
    ...
    users = _load_users()
    # Prefer already-authenticated Telegram/SaaS user so OTP does not create a second login
    cur = _current_user()
    user = None
    if cur and cur.get("username"):
        user = users.get(cur["username"]) or cur
    if not user:
        # Fallback: phone-keyed tenant (browser outside Telegram)
        un = phone
        user = users.get(un)
        if user and not user.get("active") and user.get("role") != "admin":
        ...
        if not user:
            ...
            user = {"id": nid, "username": un, "password": uuid.uuid4().hex[:12],
                    ...
                    "used": {"date": "", "count": 0}, "trial_used": 0, "balance": 0}
            users[un] = user
            _save_users(users)
    token = _issue_token(user["username"])
    ns = "u%s:%s" % (int(user["id"]), did)
    ust = DEVICES.setdefault(ns, _default_state())
    # Point global db at this namespace for _persist
    global db, CURRENT_NS
    CURRENT_NS = ns
    db = ust
    ...
    acc = next((a for a in ust["accounts"] if str(a.get("mobile", ""))[-10:] == phone), None)
    if acc:
        acc.update({
            ...
            "is_first_order": is_new if "is_new" in res else acc.get("is_first_order"),
        })
        if is_new and not acc.get("bound_offer"):
            acc["bound_offer"] = dict(next((o for o in FOD_POOL if o.get("id") == "flat180"), FOD_POOL[0]))
        ...
        if is_new:
            new_acc["bound_offer"] = dict(next((o for o in FOD_POOL if o.get("id") == "flat180"), {"id": "flat180", "flat": 180, "text": "₹180 OFF", "pct": 0}))
        ust["active_id"] = new_acc["id"]
        ust["accounts"].insert(0, new_acc)
        acc = new_acc
    if is_new:
        ust["picked_offer"] = acc.get("bound_offer")
    _persist()
    me = _auth_me(user)
    return {"ok": True, "live": res.get("live", False), "token": token,
            "account": acc, "user": me.get("user"), "plan": me.get("plan"),
            **{k: v for k, v in me.items() if k not in ("user",)}}
...
async def api_admin_users():
    ...
    for un in users.values():
        ...
        out.append({"username": un["username"], "role": un.get("role"), "plan": un.get("plan"),
                    ...
                    "balance": float(un.get("balance") or 0)})
    return {"users": out}
...
async def api_admin_users_update(data: dict = None):
    ...
    if not target:
        if data.get("password"):
            ...
            users[un] = {"id": nid, "username": un, "password": str(data["password"]),
                         ...
                         "last_seen": 0, "used": {"date": "", "count": 0}, "trial_used": 0, "balance": 0}
            _save_users(users)
    ...
    if data.get("balance") is not None:
        try:
            target["balance"] = max(0.0, float(data.get("balance") or 0))
        except Exception:
            return {"error": "bad_balance", "message": "balance must be a number"}
    if data.get("add_balance") is not None:
        try:
            target["balance"] = max(0.0, float(target.get("balance") or 0) + float(data.get("add_balance") or 0))
        except Exception:
            return {"error": "bad_balance", "message": "add_balance must be a number"}
    users[un] = dict(target)
    _save_users(users)
    return {"ok": True, "message": f"{un} updated.", "balance": float(target.get("balance") or 0)}
...
async def api_admin_settings_set(data: dict = None):
    ...
    data = data or {}
    st = _load_settings()
    if "per_order_price" in data:
        try:
            st["per_order_price"] = max(0.0, float(data.get("per_order_price") or 0))
        except Exception:
            return {"error": "bad_price", "message": "per_order_price must be a number"}
    if "admin_upi_id" in data:
        st["admin_upi_id"] = str(data.get("admin_upi_id") or "").strip()
    if "admin_upi_name" in data:
        st["admin_upi_name"] = str(data.get("admin_upi_name") or "SJ Shop").strip() or "SJ Shop"
    if "shop_name" in data:
        st["shop_name"] = str(data.get("shop_name") or "SJ Shop").strip() or "SJ Shop"
    _save_settings(st)
    return {"ok": True, "settings": st, "message": "Settings saved."}
...
async def api_bootstrap():
    ...
    try:
        u = _current_user()
        if u:
            bal = float((u.get("balance") if isinstance(u, dict) else 0) or 0)
    except Exception:
        bal = 0.0
    return {
        ...
        "balance": bal,
        "per_order_price": float(st.get("per_order_price") or 0),
        "shop_name": st.get("shop_name") or "SJ Shop",
    }
...
async def api_accounts_login_verify(data: dict):
    ...
    if res["ok"]:
        ...
        if is_new:
            new_acc["bound_offer"] = dict(next((o for o in FOD_POOL if o.get("id") == "flat180"),
                                              {"id": "flat180", "flat": 180, "text": "₹180 OFF", "pct": 0, "title": "₹180 OFF"}))
            db["picked_offer"] = new_acc["bound_offer"]
        db["accounts"].append(new_acc)
        db["active_id"] = new_acc["id"]
        _persist()
        return {"ok": True, "account": new_acc, "live": res.get("live", False),
                "fod": {"is_first_order": is_new, "offer": new_acc.get("bound_offer")}}
...
async def api_place_cod(data: dict = None):
    ...
    _fee = float(_load_settings().get("per_order_price") or 0)
    if _fee > 0:
        _bal = float((_usr or {}).get("balance") or 0)
        if _bal + 1e-9 < _fee:
            return {"ok": False, "live": True, "error": "insufficient_balance",
                    "message": f"Need ₹{_fee:.0f} wallet balance (have ₹{_bal:.0f}). Ask admin to top-up.",
                    "fee": _fee, "balance": _bal}
    h = _active_headers()
    if not h:
        return {"ok": False, "live": True, "error": "no_account",
    ...
    if amt_f <= 0:
        local_tot = 0.0
        for it in (db.get("cart") or {}).get("items") or []:
            try:
                local_tot += float(it.get("price") or it.get("final_price") or 0) * float(it.get("quantity") or 1)
            except Exception:
                pass
        if local_tot > 0:
            amt = local_tot
        else:
            return {"ok": False, "live": True, "error": "zero_amount",
                    "message": "Cart total is ₹0 — refresh cart / re-add items and try again."}
    db["cart"]["cart_session"] = cs
    db["cart"]["address"] = addr
    ...
    fee = float(_load_settings().get("per_order_price") or 0)
    ok_fee, new_bal, fee_msg = _debit_wallet(_usr, fee)
    if not ok_fee:
        return {"ok": False, "live": True, "error": "insufficient_balance",
                "message": f"Wallet short for order fee ₹{fee}. Ask admin to top-up.",
                "balance": new_bal, "fee": fee}
    _charge_order(_usr)
    return {"ok": True, "live": True, "order_num": order_num, "total": amt,
            "balance": new_bal, "fee_charged": fee,
...
async def api_order_pay_online(data: dict = None):
    ...
    # Prevent ₹0 false totals when Meesho review returns empty amounts
    try:
        amt_f = float(amt or 0)
    except Exception:
        amt_f = 0.0
    if amt_f <= 0:
        ...
        if local_tot > 0:
            amt = local_tot
            upi_amount = upi_amount or local_tot
        else:
            return {"ok": False, "live": True, "error": "zero_amount",
                    "message": "Cart total is ₹0 — refresh cart / re-add items and try again."}
    db["cart"]["cart_session"] = cs
    ...
    order = {
        ...
        "device_id": _device_id(),
        "upi_uri": upi_uri,
        "qr_image": (("data:image/png;base64," + qr_base64) if (qr_base64 and not str(qr_base64).startswith("data:")) else (qr_base64 or "")),
...
async def api_wallet_history():
    u = _current_user()
    bal = float((u or {}).get("balance") or 0) if u else 0.0
